...
This folder contains two folders, an orchestrator and an executor. The orchestrator reads from Redis and sends API calls directly to the VMs. The VM runs the code in an ephemeral docker container. That way if code escapes isolation, it's stuck in a container on an ephemeral VM.
